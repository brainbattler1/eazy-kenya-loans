-- Fix notification function to use correct column name
CREATE OR REPLACE FUNCTION public.send_notification_to_user(user_id uuid, notification_type text, title text, message text, metadata text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
BEGIN
  ...
  -- Insert the notification into a notifications table (using 'read' instead of 'is_read')
